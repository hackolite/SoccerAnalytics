...
class Pipeline:
    ...
    def __init__(self, modules: list, cfg: dict) -> None:
    ...
    def from_config(cls, config_path: str) -> 'Pipeline':
        ...
        return cls(modules=modules, cfg=cfg)
    ...
    def process_video(self, video_path: str) -> None:
        ...
        # Resolve stub path templates
        self._resolve_stub_paths(video_name)

        # Run modules
        for i, module in enumerate(self.modules, 1):
            print(f"  [{i+1}] Running module: {module.name}…")
            if module.name == 'visualization':
                output_frames = module.process(state, video_frames)
            else:
